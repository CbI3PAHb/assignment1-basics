...
def calculate_num_chunks(file_path, desired_chunk_size: int = None):
    ...
    # Get total file size in bytes
        ...
        file_size = file.tell()
        file.seek(0)
        desired_chunk_size = desired_chunk_size if desired_chunk_size is not None else 16 * 2 ** 20 #  1 MiB = 1024 KiB
        num_chunks = (file_size + desired_chunk_size - 1) // desired_chunk_size
        print(f"{file_size=}, {num_chunks=}, {desired_chunk_size=}")
    return num_chunks
...
def bpeTrainingFunction(
    ...
    with open(input_path, "rb") as file:    
        ...
        for i, (start_byte, end_byte) in enumerate(zip(chunk_boundaries[:-1], chunk_boundaries[1:])):
            # logger.debug(f"Reading chunk: {start_byte=}: {end_byte=}")
            ...
            pretokenize(chunk, byte_string_frequencies, special_tokens)
    ...
    for new_token_index in range(initial_vocab_size, vocab_size - len(special_tokens)):
        ...
        # logger.info(f"Most frequent pair is {most_frequent_pair} with freq = {pair_frequencies[most_frequent_pair]}")
        # logger.info(f"Token to insert: {new_byte_token}")
...
if __name__ == "__main__":
    vocab, merges = bpeTrainingFunction("/Users/parii-artem/Documents/assignment1-basics/data/TinyStoriesV2-GPT4-valid.txt", 512, [])
